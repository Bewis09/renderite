...
abstract class RenderiteElement<S : RenderiteDrawer<I, T, F>, P : RenderiteElement<S, P, T, F, I>, T : Any, F, I : Any>(val props: Props<P> = {}) {
    ...
    val hoverAnimation = Animator(Renderite.hoverTime, Animator.LINEAR, 0f)
    val hoverFactor
    ...
    fun renderDebug(screenDrawing: S) {
        val showUpdates = (Renderite.debugUpdateHighlightEnabled() && System.currentTimeMillis() < lastUpdateTime + 200)

        if (Renderite.debugHighlightEnabled() && !showUpdates)
            renderables.forEach { screenDrawing.fill(it.x, it.y, it.width, it.height, Renderite.debugHighlightColor()) }

        if (Renderite.debugBorderEnabled() && !showUpdates)
            renderables.forEach { screenDrawing.drawBorder(it.x, it.y, it.width, it.height, Renderite.debugBorderColor()) }

        if (Renderite.debugHighlightEnabled() && showUpdates)
            renderables.forEach { screenDrawing.fill(it.x, it.y, it.width, it.height, Renderite.debugUpdateHighlightColor()) }
    ...
    fun Div(recreateId: Number, p: Props<DivElement<S, T, F, I>>): RenderiteElement<S, *, T, F, I> {
        ...
        return addRenderable(DivElement(p)).apply {
